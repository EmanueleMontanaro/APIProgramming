...
pub fn test_try_into_f64() {
    ...
    let a : ComplexNumber = ComplexNumber::new (1.0,0.0);
    let result_a: Result<f64, &'static str> = a. try_into();
    assert_eq! (result_a, Ok(1.0));
    let b : ComplexNumber = ComplexNumber::new (1.0, 1.0);
    let result_b: Result<f64, &'static str> = b. try_into();
...
pub fn test_try_form_f64() {
    ...
    // and write test
    let real = 1.0;
    let complex:ComplexNumber = real.into();
    assert_eq!(complex.to_tuple(), (1.0,0.0));
}
...
pub fn test_deque() {
    ...
    let mut deq = VecDeque::from([a,b,c]);
    ...
    deq.push_back(c+c);
    println!("{:?}",deq);
    let search_result = deq.binary_search(&a);
    ...
    let mut vector: Vec<ComplexNumber> = deq.into_iter().collect();
    vector.sort();
    let sorted_deq: VecDeque<ComplexNumber> = vector.into();
    println!("{:?}",sorted_deq);
    let search_result = sorted_deq.binary_search(&(c+b));
